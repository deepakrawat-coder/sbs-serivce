...
if (isset($_POST['name']) && isset($_POST['id'])) {
    ...
    if (!empty($_FILES["photo"]["name"])) {
        $photo = uploadImage($conn, "photo", "testimonial");
    } else {
        $photo = $updated_file;
    }
    ...
    $update = $conn->query("UPDATE `testimonials` SET `Name` = '$name', `Product_id` = '$product_id', `Image` = '$photo',  `Title` = '$title', `Content` = '$content'  WHERE `ID` = $id");
